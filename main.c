    ...
    case 4:
        ...
        for ( i = 0; i < strlen(string); i++) {                          //the conditions for the encryption
            if(string[i]>64 && string[i]<91) {
               ...
               X=string[i];
               /*char *position_ptr = strchr(string, Substitution[i]);
               int position = (position_ptr == NULL ? -1 : position_ptr - string);
               X = position + 66; 
               printf("%d", X);
               message[i]= X; */                                        //this converted letter is stored in the string array message[i]
                                                    //the letter X is assigned to the letter value of position X along the substitution key array
            }
             else if(string[i]>96 && string[i]<123)                        //checks if the letters entered are in lowercase
            {
              char *position_ptr = strchr(string, Substitution[i]-32);
              int position = (position_ptr == NULL ? -1 : position_ptr - string);
              X = position + 66;  
              message[i]= X; 
